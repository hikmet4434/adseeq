...
export function ApifyIngestForm({ configured }: { configured: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!window.confirm(t("ingest.confirm", { terms: searchTerms.join(", "), max: maxResults, cost: maxCostUsd.toFixed(2) }))) return;
    ...
    try {
      ...
      if (!response.ok) throw new Error(t("ingest.scanFailed"));
      setError(false); setMessage(t("ingest.done", { imported: data.imported, failed: data.failed })); router.refresh();
    } catch (caught) {
      setError(true); setMessage(caught instanceof Error && caught.message !== t("ingest.scanFailed") ? caught.message : t("ingest.failed"));
    } finally { setBusy(false); }
  }

  return (
    <form onSubmit={submit} className="grid gap-3 lg:grid-cols-2">
      <input name="searchTerms" required minLength={2} maxLength={500} placeholder={t("ingest.termsPlaceholder")} className="rounded-xl border border-slate-200 px-3 py-3 lg:col-span-2" />
      ...
      <label className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-3 text-sm font-semibold"><input name="scrapeAdDetails" type="checkbox" defaultChecked /> {t("ingest.scrapeDetails")}</label>
      <label className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-3 text-sm font-semibold"><input name="includeAboutPage" type="checkbox" /> {t("ingest.aboutPage")}</label>
      <label className="text-sm font-semibold text-slate-600">{t("ingest.costLimit")}<input name="maxCostUsd" type="number" min="0.1" max="10" step="0.1" defaultValue="1" required className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-3" /></label>
      <button disabled={busy || !configured} className="self-end rounded-xl bg-violet-700 px-4 py-3 font-bold text-white disabled:opacity-40">{!configured ? t("ingest.waitingConfig") : busy ? t("ingest.scanning") : t("ingest.pull")}</button>
